    ...
    <Card className="border p-5 backdrop-blur">
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        >
          <ModeStat
            label="Best WPM"
            value={formatStat(selectedStats?.best_wpm)}
          />
          <ModeStat
            label="Avg WPM"
            value={formatStat(selectedStats?.avg_wpm)}
          />
          <ModeStat
            label="Accuracy"
          ...
          <ModeStat
            label="Tests"
            value={formatStat(selectedStats?.test_count)}
          />
        </div>
      </CardContent>
    </Card>
...
function ModeStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="border-border/60 bg-muted/45 flex flex-col gap-2 border p-3">
      ...
      <p className="text-foreground text-xl font-semibold tracking-[-0.04em]">
        {value}
      </p>
    </div>
  );
